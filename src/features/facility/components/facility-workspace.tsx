...
export function FacilityInquiries() {
  ...
  const [selected, setSelected] = useState<Inquiry | undefined>();
  function save(appointment: Appointment) { setAppointments((current) => [appointment, ...current]); setRows((current) => current.map((inquiry) => inquiry.id === appointment.inquiryId ? { ...inquiry, status: InquiryStatus.Contacted } : inquiry)); setSelected(undefined); }
  return <section><Title title="Inquiry & tindak lanjut" subtitle="Tinjau kebutuhan singkat calon pasien dan kelola tindak lanjut RS Harapan Kota." /><div style={{ display: "grid", gridTemplateColumns: selected ? "minmax(0,1.25fr) minmax(320px,.75fr)" : "1fr", gap: 18 }}><div className="card" style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}><thead><tr>{["Kontak", "Layanan", "Tujuan / keluhan singkat", "Preferensi", "Status", ""].map((item) => <th key={item} style={{ textAlign: "left", padding: 14, borderBottom: "1px solid #dce6ed", minWidth: item === "Tujuan / keluhan singkat" ? 230 : undefined }}>{item}</th>)}</tr></thead><tbody>{rows.map((inquiry) => <tr key={inquiry.id}><td style={{ padding: 14, verticalAlign: "top" }}><b>{inquiry.name}</b><br /><span className="hint">{inquiry.contact}</span></td><td style={{ padding: 14, verticalAlign: "top" }}>{inquiry.service}</td><td style={{ padding: 14, verticalAlign: "top", maxWidth: 320, lineHeight: 1.5 }}>{inquiry.purpose}</td><td style={{ padding: 14, verticalAlign: "top" }}>{inquiry.date}</td><td style={{ padding: 14, verticalAlign: "top" }}><select className="input" aria-label="Ubah status inquiry" value={inquiry.status} onChange={(event) => setRows((current) => current.map((row) => row.id === inquiry.id ? { ...row, status: event.target.value as InquiryStatus } : row))}><option>New</option><option>Contacted</option><option>Closed</option></select></td><td style={{ padding: 14, verticalAlign: "top" }}><button className="btn btn-outline btn-sm" onClick={() => setSelected(inquiry)}><CalendarClock size={15} /> Jadwalkan</button></td></tr>)}</tbody></table></div>{selected && <AppointmentForm inquiry={selected} onSave={save} />}</div><div style={{ marginTop: 24 }}><h2 style={{ color: "var(--navy)", fontSize: 20 }}>Jadwal dari inquiry</h2><AppointmentRows appointments={appointments} /></div></section>;
}
