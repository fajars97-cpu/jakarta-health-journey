...
export function PartnerLanding({ partner }: { partner: Organization }) {
  ...
  return <main className="partner-landing" style={{ "--partner-accent": profile.accent } as React.CSSProperties}>
    <section className="partner-hero" style={{ backgroundImage: `linear-gradient(90deg, #082b4af5 0%, #082b4ad9 38%, #082b4a15 78%), url(${publicBasePath}/images/partner-landing/${profile.image})` }}><div className="container"><Link href="/explore" className="partner-back">← Kembali ke katalog</Link><div className="partner-hero-content"><p><ShieldCheck size={15} /> {profile.eyebrow}</p><h1>{profile.title}</h1><span>{profile.description}</span><div><a href="#partner-inquiry" className="btn btn-coral">Mulai konsultasi <ArrowUpRight size={17} /></a><a href="#partner-packages" className="btn btn-outline">Lihat layanan</a></div><ComparisonPicker organizationId={partner.id} services={allServices} /></div></div></section>
