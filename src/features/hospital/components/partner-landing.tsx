...
export function PartnerLanding({ partner }: { partner: Organization }) {
  ...
  return <main className="partner-landing" style={{ "--partner-accent": profile.accent } as React.CSSProperties}>
    ...
    <section id="partner-packages" className="partner-section partner-tint"><div className="container"><div className="partner-heading"><div><p>PROGRAM & PAKET</p><h2>Langkah awal yang transparan.</h2></div><span>Harga merupakan ilustrasi paket MVP dan dapat berubah setelah asesmen.</span></div><div className="partner-packages">{profile.packages.map(([name, price, copy]) => <article key={name}><Sparkles size={20} /><h3>{name}</h3><strong><CurrencyPrice value={price} /></strong><p>{copy}</p><ul><li><Check size={14} />Koordinasi jadwal awal</li><li><Check size={14} />Penjelasan cakupan layanan</li></ul><ComparisonPackageButton organizationId={partner.id} service={name} /><a href="#partner-inquiry">Tanya paket <ArrowUpRight size={16} /></a></article>)}</div></div></section>
